...
- (void)multiPlug:(MultiPlug*)plug matchStatus:(int)current max:(int)max;
...
- (void)startSimpleMatch:(NSString*)userName (NSArray*)wishes;

// Start a match with friends
// Return the key (5-char string) this player should tell his friends
- (NSString*)startFriendMatch:(NSString*)userName (int)numPlayers;

// Try to join a match created by a friend
- (void)joinFriendMatch:(NSString*)userName (NSString*)key;
